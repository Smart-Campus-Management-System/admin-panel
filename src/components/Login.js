...
export default function AdminLogin() {
  ...
  const handleLogin = async (event) => {
    ...
    try {
      ...
      navigate("/dashboard/admin/dashboard-overview");
    } catch (error) {
        ...
        >
          <div className="w-full max-w-lg bg-richblack-800 p-8 rounded-xl shadow-xl transform transition-transform hover:scale-105 hover:shadow-2xl duration-500 mt-20">
            ...
            <form className="flex w-full flex-col gap-y-6" onSubmit={handleLogin}>
              ...
              <button
                  type="submit"
                  className="bg-yellow-400 mt-6 w-full max-w-md rounded-lg text-black py-3 px-6 text-lg font-medium text-white transition-all hover:scale-105 duration-300"
              >
                Log In
              </button>
            </form>
